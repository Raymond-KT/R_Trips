...
```{r library1, warning=FALSE, include=FALSE}
...
```{r library2, warning=FALSE, eval=FALSE}
...
str함수를 통해 riskfactors 데이터가 어떤 자료형과 어떤 구조로 되어 있는지  
확인하실 수 있습니다.  
...
# na.action 옵션에 따라 잔차가 다르게 나타납니다.
# 다만, riskfactors 의 변수는 차이를 바로 확인할 수는 없네요
lm(bmi~pregnant,riskfactors,na.action = na.omit)%>%resid()
lm(bmi~pregnant,riskfactors)%>%resid()
